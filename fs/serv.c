...
serve_read(envid_t envid, union Fsipc *ipc)
{
    ...
    if (err) return err;
    ssize_t cnt = file_read(o->o_file, ipc->readRet.ret_buf, MIN(req->req_n, sizeof(ipc->readRet.ret_buf)), o->o_fd->fd_offset);
    if (cnt < 0) return cnt;
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
    ...
    int err = openfile_lookup(envid, req->req_fileid, &o);
    if (err) return err;
    int cnt = file_write(o->o_file, req->req_buf, req->req_n, o->o_fd->fd_offset);
    if (cnt < 0) return cnt;
    o->o_fd->fd_offset += cnt;
    return cnt;
}
