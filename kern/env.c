...
env_alloc(struct Env **newenv_store, envid_t parent_id)
{
    ...
    static uintptr_t tf_esp = 0x210000;
	e->env_tf.tf_esp = tf_esp;
    tf_esp += 0x400000;
#else
...
region_alloc(struct Env *e, void *va, size_t len)
{
    ...
    for (uintptr_t addr = start; addr != end; addr += PGSIZE)
    {
        ...
        if (!pp) panic("region_alloc: not enough memory!\n");
        if (page_insert(e->env_pgdir, pp, (void *) addr, PTE_U | PTE_W))
        {
...
bind_functions(struct Env *e, struct Elf *elf)
{
    ...
    for (int i = 0; i != elf->e_shnum; ++i)
    {
        ...
        {
            const char *tmp = (const char *) elf + sh[elf->e_shstrndx].sh_offset;
            if (strcmp(tmp + sh[i].sh_name, ".strtab")) continue;
            str = (const char *) elf + sh[i].sh_offset;
            break;
        }
    }
    struct Elf32_Sym *st;
    for (int i = 0; i != elf->e_shnum; ++i)
    {
        if (sh[i].sh_type != ELF_SHT_SYMTAB) continue;
        st = (struct Elf32_Sym *) ((uint8_t *) elf + sh[i].sh_offset);
        for (int j = 0; j != sh[i].sh_size / sh[i].sh_entsize; ++j)
        {
            if (*(uintptr_t *) st[j].st_value) continue;
            *(uintptr_t *) st[j].st_value = find_function(str + st[j].st_name);
        }
    }
...
env_create(uint8_t *binary, size_t size, enum EnvType type)
{
    ...
    if(type == ENV_TYPE_FS) newenv_store->env_tf.tf_eflags |= FL_IOPL_MASK;
}
...
// 		ENVX(e->env_id));
    ...
    if (e->env_sigev_cnt && !(e->env_sigflags & SIG_HANDLING))
    {
        ...
        uintptr_t stack = e->env_tf.tf_esp;
        --e->env_sigev_cnt;
        sigev = e->env_sigev + e->env_sigev_cnt;
        sa = e->env_sa + sigev->sigev_signo;
        stack -= 4;
        *(uintptr_t *) stack = e->env_tf.tf_eip;
        ...
        *(void (**)()) stack = sa->sa_restorer;
        ...
        e->env_tf.tf_eip = (uintptr_t) sa->sa_handler;
        e->env_tf.tf_esp = stack;
    }
