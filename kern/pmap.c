...
mem_init(void)
{
	...
	// LAB 8: Your code here.
    boot_map_region(kern_pgdir, UENVS, ROUNDUP(NENV * sizeof(*envs), PGSIZE), PADDR(envs), PTE_U);
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here:
    boot_map_region(kern_pgdir, KERNBASE, -(int32_t) KERNBASE, 0, PTE_W);
...
page_free(struct PageInfo *pp)
{
    ...
    if (pp->pp_ref) panic("page_free: page is in use!\n");
    if (pp->pp_link) panic("page_free: double free!\n");
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
    ...
    {
        size_t off = i * PGSIZE;
        *pgdir_walk(pgdir, (uint8_t *) va + off, 1) = (pa + off) | perm | PTE_P;
    }
}
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
    ...
    if ((*pte & PTE_P) && PTE_ADDR(*pte) != page2pa(pp)) page_remove(pgdir, va);
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
    ...
    if (start + len > ULIM)
    {
        user_mem_check_addr = ULIM > start ? ULIM : start;
        return -E_FAULT;
    }
    perm |= PTE_P;
    pte_t *pte;
    uintptr_t end = ROUNDUP((uintptr_t) va + len, PGSIZE);
    for (uintptr_t addr = start; addr != end; addr = ROUNDDOWN(addr + PGSIZE, PGSIZE))
    {
        if (!page_lookup(env->env_pgdir, (void *) ROUNDDOWN(addr, PGSIZE), &pte) ||
            (*pte & perm) != perm)
        {
